...
async function insertCrew(transaction: Transaction, name: string, reward: number) {
  const result = await transaction.queryArray
    `SELECT * from crews where name = ${name}`;

  if (result.rows.length == 0) {
    await transaction.queryArray
      `INSERT INTO crews VALUES (nextval('crews_id_seq'), ${name}, ${reward})`;
  }
}
...
await transaction.queryArray
  `CREATE TABLE IF NOT EXISTS crews (id SERIAL, name VARCHAR(256), reward BIGINT)`;
